...
Like us, songbird brains contain two interacting systems for vocal communication: one for song production and one for perception, with auditory circuits critical for recognition and learning. My research found that the elements of a bird's song (syllables) are learned, but how those elements are placed in order (syntax) is shaped by species genetics. The separation of syllables and syntax is mirrored in the brain by the encoding of syllable morphology by individual neurons, and the encoding of syntax by neural circuits.
